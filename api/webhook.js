...
function getBuiltInToppingNames(product) {
  const names = new Set();
    ...
    (draft, draftIndex) => {
      ...
      return {
        ...
        phone:
          serialized.phone || '',
        contactName:
          serialized.contactName || '',
        deliveryNote:
          serialized.deliveryNote || '',
...
function buildStaffOrderSummary(orderDoc) {
  ...
  for (const subOrder of subOrders) {
    ...
    for (
      const [itemIndex, item] of (subOrder.items || []).entries()
    ) {
      lines.push(
        `${itemIndex + 1}. ${item.name} ${item.size || ''} ×${item.qty}`
      );
      ...
      for (
        const request of item.specialRequests || []
      ) {
        const requestSubtotal =
    ...
    if (
      ...
      if (subOrder.deliveryNote) {
        lines.push(
          `📝 ${subOrder.deliveryNote}`
        );
      }

      if (
        subOrder.contactName ||
        subOrder.phone
      ) {
        lines.push(
          `☎️ ${[
...
function buildFinalDraftText(draftDoc) {
  ...
  drafts.forEach((draft, draftIndex) => {
    ...
    for (const [itemIndex, item] of (draft.items || []).entries()) {
      ...
      output.push(
        `${itemIndex + 1}. ${item.name} ${item.size} ×${item.qty}　$${subtotal}`
      );
      ...
      for (
        ...
        output.push(
          `　📝 特殊需求：${request.label}` +
          (
            request.pricingStatus === 'unconfigured'
              ? '（加價未設定）'
              : ''
          )
        );
    ...
    if (
      ...
      output.push(
        `📝 外送備註：${draft.deliveryNote}`
      );
    }

    if (
      draft.fulfillment === '外送' &&
      (draft.contactName || draft.phone)
    ) {
      output.push(
        `☎️ 聯絡：${[
          draft.contactName,
      ...
      for (const event of events) {
        ...
        try {
          ...
          const submittedText = [
            buildDraftReply(drafts),
            '',
            `✅ 已送出｜${(formalResult.displayOrderNos || []).join('、') || formalResult.orderId}`,
            '※ 實際金額依發票開立金額為主。'
          ].join('\n');
...
function quantityFromSegment(
  ...
  const suffixMatch =
    text.match(
      /(?:[xX×*]\s*)(\d+|[一二兩三四五六七八九十]+)\s*(?:杯|瓶)?(?=\s*(?:正常糖|少糖|半糖|微糖|一分糖|無糖|正常冰|少冰|微冰|去冰|完全去冰|常溫|溫|熱|加|[，,;；]|$))/
    );
...
function sizeFrom(segment, product) {
  ...
  if (/(^|[^A-Za-z])M([^A-Za-z]|$)|中杯|小杯/i.test(segment)) return 'M';
...
function extractDeliveryMeta(raw) {
  ...
  const phoneMatch = text.match(
    /(?:聯絡電話|聯絡手機|電話|手機)\s*[:：]?\s*(09\d{2}[-\s]?\d{3}[-\s]?\d{3})\s*([^\n，,。]*)?/i
  );
  ...
  if (phoneMatch) {
    ...
    contactName = String(phoneMatch[2] || '')
      .trim()
      .replace(/[，,。].*$/, '');
  }
  ...
  if (addressMatch) {
    const fullAddressLine =
      String(addressMatch[1] || '').trim();

    const mainAddress =
      extractAddress(fullAddressLine);
    ...
    if (mainAddress) {
      deliveryNote =
        fullAddressLine
          .replace(mainAddress, '')
          .replace(
            /[，,、\s]*(?:聯絡電話|聯絡手機|電話|手機)\s*[:：]?\s*09\d{2}[-\s]?\d{3}[-\s]?\d{3}\s*[^，,。]*/i,
            ''
          )
          .trim()
          .replace(/^[，,、\s]+/, '')
  ...
  if (!deliveryNote) {
    const placeMatch = text.match(
      /(?:^|[，,\s])([\u4e00-\u9fa5A-Za-z0-9]{2,20}(?:新村|社區|大樓|公司|學校|醫院|診所|工廠|門市))(?:[，,\s]|$)/
    );

    if (placeMatch) {
      deliveryNote =
        String(placeMatch[1] || '').trim();
    }
  }
  ...
  const locationLabel =
    introMatch
      ? String(introMatch[1] || '').trim()
      : '';

  if (
    locationLabel &&
    !deliveryNote.includes(locationLabel)
  ) {
    deliveryNote =
      deliveryNote
        ? `${locationLabel}｜${deliveryNote}`
        : locationLabel;
  }
...
function specialRequestsTotal(requests) {
...
function specialRequestsFromSegment(
  segment,
  product
) {
  const requests = [];
  const text = String(segment || '');
  const isCheeseFoamDrink =
    String(product?.name || '')
      .includes('奶蓋');

  if (
    /(雙倍奶蓋|奶蓋雙倍|雙份奶蓋|奶蓋雙份|兩份奶蓋)/.test(text)
  ) {
    if (isCheeseFoamDrink) {
      requests.push({
        type: 'extra_cheese_foam',
        label: '雙倍奶蓋',
        detail: '原有1份＋加1份奶蓋',
        qty: 1,
        unitPrice: 5,
      ...
      requests.push({
        type: 'double_cheese_foam',
        label: '雙倍奶蓋',
        detail: '非奶蓋飲品的雙倍奶蓋需人工確認份數',
        qty: 0,
        unitPrice: 0,
        subtotal: 0,
...
function specialRequestIssues(requests) {
  const issues = [];
...
function parseOrderBlock(raw) {
  ...
  hits.forEach((hit, index) => {
    ...
    const toppingSegment =
      segment.slice(hit.len);

    const builtInToppings =
      getBuiltInToppingNames(
        product
      );

    const toppings =
      findToppingsInSegment(
        toppingSegment
      ).filter(topping => {
        if (
          !builtInToppings.has(
        ...
        const escaped =
          topping.name.replace(
            /[.*+?^${}()|[\]\\]/g,
            '\\$&'
          );

        return new RegExp(
          `(?:加|追加|再加|多加|另外加|額外加)\\s*${escaped}`
        ).test(
          toppingSegment
        );
      });
...
function buildDraftReply(drafts) {
  ...
  drafts.forEach((draft, draftIndex) => {
    ...
    draft.items.forEach(item => {
      ...
      output.push(
        `${itemNo}. ${item.name} ${item.size} ×${item.qty}　$${itemSubtotal}`
      );
  ...
  output.push(
    `🥤 ${summary.drinkCount}杯　💰 $${summary.finalTotal}`
  );

  if (
    summary.promotion.freeDrinkCount > 0
  ) {
    output.push(
      `🎁 買10送1 ×${summary.promotion.freeDrinkCount}，已折 $${summary.promotion.discountAmount}`
    );
  }

  if (
    summary.promotion.shouldRemindAddOne
  ) {
    output.push(
      '🎁 再加1杯可多享一次買10送1'
    );
